...
**The Bottleneck:** If the Checkout team wanted to change the data format, they had to file a ticket with the ESB team and wait six weeks for a WSDL update.
...
In a microservices architecture, the network (the pipe) should do nothing but transport packets. It should be HTTP, gRPC, or a simple message broker. The intelligence - the routing decisions, business rules, the data mapping - must reside within the service itself.
...
A robust heuristic for the Senior Architect is the industry standard **Two-Week Rewrite Rule**: A microservice should be small enough that a standard "Two-Pizza Team" (6-8 engineers) could rewrite it from scratch in two weeks without disrupting the rest of the system.
...
Adopting the team-centric sizing principles from Team Topologies, the VaquarKhan (Khan) Granularity Protocol™ quantifies these constraints to help architects optimize for the "Goldilocks Zone" where the team can hold the entire domain model in their working memory.
...
**Solution:** Do not rely on static analysis (who calls whom). Rely on Temporal Coupling (who changes with whom).

Building on the forensic code analysis concepts popularized by Adam Tornhill in his book *Your Code as a Crime Scene* (2015), this recipe demonstrates how to identify temporal coupling - where files change together over time to detect hidden architectural dependencies.
